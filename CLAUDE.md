...
## Estrutura
- `src/app/` — `layout.tsx` (ClerkProvider), `page.tsx` (redirect por papel / tela de login na raiz), `vendedor/` (client), `gestor/` (server), `api/vendedor/perfil/` (promove loja/telefone do autocadastro pra `publicMetadata`), `api/gestor/relatorio-seguros/` (gestor-only, cruza `seguros_indicacao_movida` x `captacoes` por placa e devolve `.xlsx` mensal por loja), `sign-in`/`sign-up` (redirecionam pra `/`, o login real é a raiz).
