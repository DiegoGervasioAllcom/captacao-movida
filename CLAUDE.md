...
## Estrutura
- `src/app/` — `layout.tsx` (ClerkProvider), `page.tsx` (redirect por papel / tela de login na raiz), `vendedor/` (client), `gestor/` (server, tabela paginada e filtrada no banco via `searchParams` `pagina`/`busca`; métricas de transmissões emitidas hoje/mês vêm de `seguros_indicacao_movida`), `api/vendedor/perfil/` (promove loja/telefone do autocadastro pra `publicMetadata`), `api/gestor/relatorio-seguros/` (gestor-only, sincroniza as planilhas, cruza `seguros_indicacao_movida` x `captacoes` por placa e devolve `.xlsx` mensal por loja), `api/gestor/sincronizar-seguros/` (POST gestor-only, sincroniza apenas o dia ou mês solicitado e retorna a contagem agregada de emitidas), `api/gestor/captacoes-export/` (gestor-only, devolve todas as captações que batem com a busca, sem paginação, pro CSV), `sign-in`/`sign-up` (redirecionam pra `/`, o login real é a raiz).
