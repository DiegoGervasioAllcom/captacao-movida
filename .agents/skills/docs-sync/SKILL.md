...
| Mudança | Documentos |
|---------|-----------|
| Regra de arquitetura, novo arquivo importante, novo agente/skill | `CLAUDE.md` + `AGENTS.md` (regras / Estrutura / tabela de agentes) |
...
| Gotcha de trabalho para agentes (receita, armadilha) | A skill correspondente em `.claude/skills/` e `.agents/skills/` |
...
3. Confira também: `CLAUDE.md`/`AGENTS.md` citam arquivos que ainda existem? Os agentes/skills de `.claude/`, `.codex/agents/` e `.agents/skills/` citam paths/funções que ainda existem? (`grep` pelos nomes).
