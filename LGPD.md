...
A tabela `seguros_indicacao_movida` recebe a placa (e dados de venda de seguro associados) a partir de colunas preenchidas manualmente pelo time de seguros nas mesmas 3 planilhas Google Sheets que já recebem as captações (seção 7). A rota `src/app/api/gestor/relatorio-seguros` sincroniza essas colunas para o Supabase sempre que o gestor gera o relatório mensal. Os botões das métricas de transmissões usam a rota `src/app/api/gestor/sincronizar-seguros` para sincronizar somente o dia ou mês solicitado, sem gerar arquivo e retornando ao navegador apenas a contagem agregada de registros cuja `data_venda` esteja no período, independentemente do status.
