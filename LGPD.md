...
| CPF do cliente | Dado pessoal de identificação direta | Importado do ViaNuvem/Unico Auto (nunca coletado pelo formulário de indicação do vendedor) |
| E-mail do cliente | Dado pessoal / contato | Importado do ViaNuvem/Unico Auto (nunca coletado pelo formulário de indicação do vendedor) |
...
| CPF do vendedor | Dado do colaborador / identificação direta | Informado pelo próprio vendedor no autocadastro (`publicMetadata.cpf` no Clerk) — **não** é replicado em `captacoes` nem enviado ao webhook/planilhas (seção 7) |
| E-mail e senha do vendedor | Dado do colaborador / credencial de acesso | Informados pelo próprio vendedor no autocadastro, restrito ao domínio corporativo `@movida.com.br`; geridos inteiramente pelo Clerk (autenticação) — não ficam armazenados em `captacoes` |
...
**CPF e e-mail do cliente** entraram no schema para suportar a importação automática do ViaNuvem/Unico Auto (`vianuvem-import/`) e alimentam as colunas correspondentes das 3 planilhas do Google Sheets (seção 7) — finalidade definida: preencher os campos CPF/E-mail que a planilha gerencial já tinha. Só são preenchidos para captações com `vendedor_id = "vianuvem"`; captações do formulário de indicação do vendedor continuam sem esses 2 campos.

**Dados do próprio vendedor (autocadastro):** desde que o portal ganhou cadastro self-service (tela de login, aba "Criar conta"), o vendedor informa nome, e-mail, telefone, CPF e senha diretamente. Isso é dado de **colaborador**, tratado sob um regime distinto do dado do cliente na tabela 2 acima — a base legal aqui é a relação de trabalho/parceria com a loja, não consentimento do cliente final. E-mail e senha ficam inteiramente sob custódia do Clerk (autenticação); nome, loja, telefone e CPF ficam em `publicMetadata`; telefone é o único desses campos do vendedor replicado em `captacoes` (`vendedor_telefone`), porque acompanha cada indicação registrada — **CPF do vendedor não é replicado em lugar nenhum**, fica só no Clerk (finalidade: identificação do colaborador, não alimentar planilha ou indicação). O cadastro também exige e-mail do domínio corporativo `@movida.com.br` (validado no cliente, `emailCorporativoValido` em `src/lib/validation.ts`) — reforça que a conta pertence a um colaborador Movida, mas não é, por si só, um controle de segurança contra bypass no navegador.
...
- **CPF do cliente**, quando presente (origem ViaNuvem), deve ter prazo de retenção igual ou mais curto que os demais campos, dado que seu uso hoje é preencher a planilha gerencial (seção 7) e não há atendimento contínuo baseado nele — a placa (não o CPF) é a chave usada para deduplicação.
- **CPF do vendedor** segue o prazo de retenção do vínculo do colaborador com a loja (mesmo regime de nome/telefone/loja em `publicMetadata`), não o prazo mais curto acima — sua finalidade é identificação do colaborador, distinta da planilha gerencial. Deve ser eliminado do Clerk quando o colaborador deixar de atuar como vendedor.
