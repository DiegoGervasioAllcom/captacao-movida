...
A tabela `seguros_indicacao_movida` recebe a placa (e dados de venda de seguro associados) a partir de colunas preenchidas manualmente pelo time de seguros nas mesmas 3 planilhas Google Sheets que já recebem as captações (seção 7). A rota `src/app/api/gestor/relatorio-seguros` sincroniza essas colunas para o Supabase sempre que o gestor gera o relatório mensal. As métricas de transmissões apenas consultam no banco a contagem agregada dos registros cuja `data_venda` esteja no dia ou mês atual, independentemente do status; nenhum dado individual é enviado ao navegador.
