...
2. Verificação por e-mail: `prepareEmailAddressVerification({ strategy: "email_code" })` → etapa "codigo" → `attemptEmailAddressVerification({ code })` → exige `status === "complete"` → `setActive(...)`.
